...
| `identity/` | the visual identity, generated by `render_logo.py` |
| `scripts/ci.sh` | the gate |
| `docs/` | design records |
| `REPORT_*.md` | one per build, including what each disproved |
...
`REPORT_*.md` files are the build record; each one ends with what that build
disproved about its own plan, which is usually the most useful part.
