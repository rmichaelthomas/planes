...
| **A lenient read for a host's rules**: the rules it can use, plus each rule set aside with its reason | Not built. **Decided** (architect, September 14, 2026): a browser's reader file should not switch off every rule over one error, as Koncord's does today (Koncord v1.14 §298.2). Set aside only what can't be used; where setting a rule aside changes what is allowed, keep the refusal — a broken exception is set aside, and in a permit/forbid clash the forbid stays. An option a host chooses, in Python, JS and Swift under the agreement tests; 5xFive's compliance checks stay all-or-nothing | — |
