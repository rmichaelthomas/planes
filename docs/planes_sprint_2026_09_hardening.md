...
| Koncord | koncord-shared-agency #40, merged | The compiled list and page watch cover addresses, as two WebKit triggers per address (its URL filter has no `|`). `send` rules were left out at this PR. Pinned at `sprint-b-2026-09.1`. 783 tests pass. Parity: Etsy and Airbnb fail on pixels as already accepted. The New York Times fails on fonts (12 vs 20), on `main` as well. The architect accepted it |
| Koncord | koncord-shared-agency #41, merged | B1 taken up: a form, an upload, a beacon and a page's request with a body are checked as `send`, and refused before they go. The page watch refuses a page's own send; WebKit's block list can't tell a send from a fetch, so it answers fetches only |
| Koncord | koncord-shared-agency #42, merged | B3 taken up. `contradicts` had not been: a pair refused a document Koncord fetched while its block list let the same address load. Koncord now builds the list and the page watch from `HostRuleSet.check`'s answer at each address a rule names, which also ended two older splits (a same-kind `supersedes` Planes drops, and a wide superseding permit clearing a broader forbid). A contradiction's reason names the pair. B4, surface format 2 and #143 don't reach Koncord |
