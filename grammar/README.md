...
|---|---|---|
| `vocabulary.json` | Source of truth for the language's vocabulary — token classes, reserved words, builtin names, effect kinds, the field-name token set, and the seven positional words no other table records. `lexer.py` and `parser.py` load it at import time. | **Hand-edited.** This is the one file in this directory a person changes directly. |
...
| `core.json` | The subset a self-hosting implementation may use (root `README.md`) — the port surface a second host must implement to run `grammar/interp.planes` **and every file it reaches through `use`**. Declared here as a single source of truth, the same role `vocabulary.json` plays for the full surface, and enforced by `core_check.py`, which fails when any file in that graph uses a keyword or builtin outside it. `core_check.py` also holds this file against `vocabulary.json` itself: every keyword and builtin the vocabulary declares must be in a core list or in the matching `excluded_` map with a reason, and the `size` strings must parse to the real counts. That guard exists because this file drifted exactly as a hand-edited file does — `root` was outside the core with no reason recorded, and nothing said so. **It drifted a second time, and differently.** `core_check.py` read one file while the artifact was five, so `when` sat in `excluded_keywords` while `grammar/lexer.planes` — reached through `use parser` — needed it sixteen times, in the loop every character of every program passes through. A host built to the core as declared could not have run the interpreter the core was the port surface for. Both halves are closed now: the checker follows `use`, and `when` is back in the core, where `reports/CORE_SUBSET.md` §1.1 had it from the start (§4a records the round trip). | **Hand-edited.** `grammar_gen.py` never reads or writes it; `core_check.py` reads it directly. |
...
once for `interp.planes`, once for `json.planes`; each run follows `use`, so the
first covers `parser.planes`, `lexer.planes` and `vocabulary.planes` beneath it
and all five self-hosted files are checked), the two coverage reports,
