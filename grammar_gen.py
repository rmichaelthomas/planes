...
grammar/vocabulary.planes from the actual source (ruling D1: generated,
not hand-authored).
...
Never writes to grammar/vocabulary.json. That file is source-of-truth
(ruling D2), hand-edited, and read here like any other input —
grammar/vocabulary.planes is a pure Planes-syntax projection of it, the
same relationship rules.json and errors.json have to parser.py and the
repo's *.py files (grammar/README.md). A lexer written in Planes needs
the keyword/builtin/operator vocabulary inline, not read from JSON at
runtime (`host.read` returns text; JSON parsing is reached only through
`ask`, never `read` — verified against host.py) — generating it here
keeps that lexer's own effect surface clean of a `read` it does not
otherwise need, and keeps vocabulary.json the single authored copy.
"""
import ast
import difflib
import glob
...
VOCAB_JSON_PATH = os.path.join(REPO, "grammar", "vocabulary.json")
...
# grammar/vocabulary.json's own OP pattern is already longest-first:
# `->|==|!=|<=|>=|[+\-*/=<>().,;:\[\]{}@]` -- the two-and-more-character
# spellings before the single characters they'd otherwise be mistaken for
# a prefix of. A regex-free lexer (lexer.planes) has to try them in this
# same order, by hand, so the order is data this emitter must preserve,
# not something the consumer re-derives.
_OP_MULTI = ["->", "==", "!=", "<=", ">="]
_OP_SINGLE = list("+-*/=<>().,;:[]{}@")
...
def _planes_list(items):
    return "[" + ", ".join(_planes_string(i) for i in items) + "]"


def generate_vocabulary_planes():
    """grammar/vocabulary.json's keywords, builtins, and operator
    spellings, projected as Planes zero-argument functions -- inline
    data a lexer written in Planes can `use vocabulary` and call, with
    no `read` effect and nothing for its own effect surface to declare.

    Functions, not top-level bindings: `use` only hoists FuncDef (and
    Foreign) across files (interp.py's Interpreter.hoist) -- a plain
    `keywords = [...]` in a non-entry module is never executed at all,
    since run_file only executes a non-entry file's Use statements, not
    its other top-level statements. Confirmed empirically before writing
    this: a plain top-level list binding in a `use`d file raises
    unknown-name from the importer; the same binding wrapped as
    `to keywords: give [...]` and called as `keywords` resolves. This is
    the only shape that actually crosses a module boundary.
    """
    with open(VOCAB_JSON_PATH, encoding="utf-8") as f:
        vocab = json.load(f)

    keywords = [e["word"] for e in vocab["keywords"]]
    builtins = [b["name"] for b in vocab["builtins"]]
    operators = _OP_MULTI + _OP_SINGLE

    lines = [
        'note: from "generated by grammar_gen.py from grammar/vocabulary.json '
        '-- do not hand-edit"',
        "",
        "to keywords:",
        f"  give {_planes_list(keywords)}",
        "",
        "to builtins:",
        f"  give {_planes_list(builtins)}",
        "",
        "# Longest-first -- a regex-free lexer must try -> == != <= >= before",
        "# falling back to a single character, exactly as vocabulary.json's",
        "# own OP pattern does.",
        "to operators:",
        f"  give {_planes_list(operators)}",
        "",
    ]
    return "\n".join(lines)
...
def main():
    ...
    vocab_planes_text = generate_vocabulary_planes()
    ...
    if not check:
        ...
        _write_text(VOCAB_PLANES_PATH, vocab_planes_text)
        ...
        print(f"wrote {VOCAB_PLANES_PATH}")
        return 0
    ...
    for path, generated, label in (
            ...
            (VOCAB_PLANES_PATH, vocab_planes_text, "grammar/vocabulary.planes")):
