...
case "shapes": EffectSurfaceCommand.run(rest)
case "shapes-fn": EffectSurfaceFunctionsCommand.run(rest)
case "shapes-deriv": EffectSurfaceDerivationCommand.run(rest)
...
case "render-from-fields": RenderFromFieldsCommand.run(rest)
case "shapes-cli": EffectSurfaceToolCommand.run(rest)
