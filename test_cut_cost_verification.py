"""test_cut_cost_verification.py — Horizon Phase 1: `_cut`'s per-`mk`
cost, the build prompt's own §6.2 verification gate (assertions A-F).

THE ONE DEVIATION FROM THE BUILD PROMPT, STATED WHERE THE NEXT BUILD READS
IT (the same deviation test_retention_tail_verification.py and
test_world_kernel_verification.py record for their own builds, for the
identical reason). §6.2 names this file `scripts/verify-cut-cost.py`. This
repo's own `test_gate.py` (C6, Ruling 3) hard-fails `scripts/ci.sh` on ANY
`verify_*`/`verify-*` script anywhere in the tree — "a verification script
graduates into a suite or is deleted when its build merges." This file's
own docstring already named the remedy (graduate into `scripts/run_suites.py`
the way `test_retention_tail_verification.py` did) but shipped under the
`scripts/verify-cut-cost.py` name anyway, so `test_gate.py` caught it after
the fact — this rename is that graduation, finally carried out, not a new
build.
...
# Pinned to fixed, immutable SHAs — NOT "main" (a branch name, which drifts
# every time a later PR merges). This file graduated from a one-off
# verify-cut-cost.py into a PERMANENT suite member (PR #94), and a
# permanent member that diffs against a moving ref only ever verifies
# anything at the exact moment its own PR merges: found live, running this
# gate for Horizon Phase 2 Build 2 — `test_f_only_expected_files_changed`
# failed on an entirely unrelated branch because "main" had, correctly,
# moved on. BASE_REF/HEAD_REF below are this PR's (#89) own base and
# merge commit — a fixed historical range that stays true forever,
# regardless of which branch or HEAD state runs the suite.
BASE_REF = "02010fd"  # the pre-this-build state
HEAD_REF = "4039685"  # this build's own tip (PR #89's merge commit)
WINDOW = 5
...
def _diff_hunks_out_of_scope(path, allowed_spans):
    ...
    if not diff.strip():
        return []
    # HEAD_REF's own content, not the current working file: a permanent
    # suite member checking a FIXED historical diff must locate its span
    # markers in that SAME fixed state, or a later, unrelated refactor
    # that moves those markers around would silently point this check at
    # the wrong lines without ever failing it.
...
def test_f_only_expected_files_changed():
    """A fixed, permanent historical record of PR #89's (BASE_REF..HEAD_REF)
    own diff, not a live "what have I touched" gate — see BASE_REF/HEAD_REF's
    own comment for why this must be two fixed commits, never a moving
    branch name or the current working tree. The file was still named
    `scripts/verify-cut-cost.py` at HEAD_REF; its later rename to
    `test_cut_cost_verification.py` (PR #94, the graduation this docstring
    already describes) happened in commits after this fixed range and is
    not part of what this specific check verifies."""
    diff = subprocess.run(
        ["git", "diff", "--name-only", BASE_REF, HEAD_REF], cwd=REPO,
