"""The Swift lexer, checked against lexer.py.

The Swift counterpart of test_js_lexer.py, with `planes-swift` in place of
`node js/cli.mjs`. swift/Sources/Planes/Lexer.swift is a port of lexer.py's
tokenize(). This test tokenizes every .planes file in the repo with both
implementations and compares the token streams — the same check
test_lexer_in_planes.py performs for the Planes lexer, reusing its canonical
form: a list of (kind, value, line) per token (A.3, reuse the existing form, do
not invent a fourth).

Target: 100% agreement. Per-file results are reported for anything short of it.
Included is grammar/lexer.planes — the Planes lexer source, string escapes and
all — against lexer.py.

The last section has no JavaScript counterpart. The Swift lexer scans unicode
scalars by hand instead of compiling the vocabulary's regular expressions, so
the places Python's `re` and `str` are Unicode-aware — `\\d`, `str.strip()`,
code-point positions — are driven directly, over every character Python counts
as a digit or as whitespace.
"""
import glob
import json
import os
...
def test_the_swift_lexer_tokenizes_the_planes_lexer_source_identically():
    ...
    identically between Lexer.swift and lexer.py."""
    f = "grammar/lexer.planes"
    assert py_tokens(f) == swift_tokens(f)


def test_the_swift_lexer_tokenizes_the_interpreter_source_identically():
...
def test_an_unrecognized_escape_is_a_syntax_error_with_the_same_message():
    ...
    with tempfile.TemporaryDirectory() as d:
        got = _swift_tokens_src(d, src)
    assert isinstance(got, dict) and got["error"] == "PlanesSyntaxError", got
    assert got["message"] == py_msg, f"swift={got['message']!r}\npy={py_msg!r}"
...
# ======================================================= Unicode: Python's semantics, not Swift's
...
def _agree_src(src):
    ...
    if isinstance(want, list) and isinstance(got, list):
        ...
        assert want == got, f"swift={got!r}\npy={want!r}"


def test_every_python_digit_is_a_number_and_nothing_else_is():
    """`\\d` in a Python str pattern is Unicode category Nd, at Python's Unicode
    version. Lexer.swift carries that set as a table; Swift's own tables are a
    Unicode version ahead, so its extra digits (U+11DE0...U+11DE9 in Unicode 17)
    are driven here too, and must not lex as NUMBER."""
    ...
    try:
        ...
        raise AssertionError(
            f"{e}\n(Lexer.swift's digit table is Python 3.14's, Unicode 16.0; this "
            f"Python is Unicode {unicodedata.unidata_version})") from None


def test_python_whitespace_is_what_strip_and_indentation_count():
    """str.strip() and str.lstrip() strip Python's isspace set — not Swift's,
    and not JavaScript's trim(). Each such character is used as indentation, as
    trailing space, as a whole line, and inside a line (where only space and tab
    are WS, and anything else is a skipped stray character)."""
    ...
    try:
        ...
        raise AssertionError(
            f"{e}\n(Lexer.swift's whitespace set is Python 3.14's, Unicode 16.0; this "
            f"Python is Unicode {unicodedata.unidata_version})") from None


def test_non_ascii_text_lexes_by_code_point():
    """Stray characters are skipped one code point at a time, and a combining
    mark after a quote is its own code point — never folded into the quote the
    way a Swift Character would fold it."""
    _agree_src("\n".join([
        'café = "naïve 👨‍👩‍👧 𝕏"',
        "é1 = x́y",
        'show "a"́ + "é"',
...
def test_line_endings_and_a_byte_order_mark():
    ...
    leading byte-order mark is kept, as Python's utf-8 codec keeps it."""
    _agree_src("x = 1\r\n  y = 2\r\nz\r= 3\r")
    _agree_src("﻿  x = 1\n  y = 2\n")
    _agree_src(" x = 1 \n\x0c  y\x0b= 2\n")
