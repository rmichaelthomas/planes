"""Exact rationals in Swift, checked against planes_num.py.

The Swift counterpart of test_js_num.py, with `planes-swift` in place of
`node js/cli.mjs`. Swift has no arbitrary-precision integer and Double is a
float, so swift/Sources/Planes/PlanesNumber.swift represents a number as a
Fraction of two BigInts (swift/Sources/Planes/BigInt.swift). This test drives
the same operations through planes-swift and planes_num.py and compares the
rendered text, including the cases where floating point visibly diverges from
exact arithmetic: 0.1 + 0.2, 1 / 3, round-half-away, and a denominator past the
bound.
"""
import json
import subprocess
