"""The Swift analyser's derivation graph, checked against shapes.py.

The Swift counterpart of test_js_shapes_derivation.py, with `planes-swift` in
place of `node js/cli.mjs`. The effect surface says WHAT a program touches; the
derivation graph says WHERE a value came from — the static analogue of `why`.
...
The programs are shared with test_swift_shapes.py (its inline and its non-ASCII
programs) plus the derivation-specific cases from test_shapes.py: a parameter
origin, an empty origin for a bare literal, a literal reached through a variable,
a re-escaped quote in a literal label, and the unknown provenance node widening
produces at a branch join — and, with no JavaScript counterpart, literal labels
and origins carrying non-ASCII text.
"""
import json
import os
import subprocess
...
from test_swift_shapes import INLINE, NON_ASCII


def _swift_deriv(path):
    r = subprocess.run(command("shapes-deriv", path), cwd=REPO, capture_output=True, text=True)
    if r.returncode != 0:
...
# Derivation-specific programs, on top of INLINE and test_swift_shapes.py's
# non-ASCII programs.
DERIV_CASES = INLINE + NON_ASCII + [
    ...
    # non-ASCII literal labels re-escape, and names order the origins
    'use http\nlet u = "\u00e9\\"\U0001f600\\n"\nx = ask u',
    ('use http\nto send of zeta, alpha:\n  give ask "https://\u00e9/" + zeta + alpha\n\n'
     'x = send of "\U0001f600", "e\u0301"\n'),
    # a foreign's claim through a non-ASCII argument
    'foreign f of u from "m.\u00e9" doing ask u\nlet v = "https://\u00fc"\nr = f of v\n',
]
...
def test_origins_reaches_a_named_parameter():
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        with open(p, "w", encoding="utf-8", newline="") as fh:
            fh.write(src)
        sw = _swift_deriv(p)
    asks = [e for e in sw if e["kind"] == "ask"]
