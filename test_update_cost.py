...
Four assertions, the minimum this build's verification gate requires:
  ...
  D. Non-interference -- no file outside scripts/, benchmarks/, reports/, and
     this file differs from `main` (invariant 1: this build changes nothing
     it measures).
...
ALLOWED_PREFIXES = ("scripts/", "benchmarks/", "reports/")
# derived-surface-audit.md is machine-regenerated by
# scripts/check_derived_claims.py (part of `scripts/ci.sh`) from the repo's
# own file tree, every gate run, on every branch -- it is not this build's
# source, and NOT letting it update would itself be the staleness bug the
# derived-surface-audit class exists to catch (it lists every file in
# scope, and this build adds two).
ALLOWED_EXACT = ("test_update_cost.py", "derived-surface-audit.md")
...
def test_no_file_outside_the_declared_scope_differs_from_main():
    ...
    assert not stray, (
        "files outside scripts/, benchmarks/, reports/, and "
        "test_update_cost.py differ from main: " + ", ".join(stray))


def test_no_untracked_file_outside_the_declared_scope():
    result = subprocess.run(
    ...
    for line in result.stdout.splitlines():
        if not line.startswith("??"):
            continue
        f = line[3:].strip()
        if f in ALLOWED_EXACT or any(f.startswith(p) for p in ALLOWED_PREFIXES):
            continue
        stray.append(f)
    assert not stray, "untracked files outside the declared scope: " + ", ".join(stray)
